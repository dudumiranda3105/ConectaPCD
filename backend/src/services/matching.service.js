...
exports.MatchingService = {
     ...
     */
    async calcularMatch(candidatoId, vagaId) {
        ...
        const mitigacoesBarreiras = new Map(); // barreiraId -> melhor eficiencia
        ...
        const necessidadesDerivadas = candidato.subtipos.flatMap(cs => cs.barreiras.flatMap(csb => {
     ...
     */
    async salvarMatch(match) {
        try {
            // Verificar se a tabela existe antes de tentar usar
            await prisma_1.prisma.$queryRaw `SELECT 1 FROM "MatchScore" LIMIT 1`;
            await prisma_1.prisma.matchScore.upsert({
                where: {
                    candidatoId_vagaId: {
     ...
     */
    async buscarMelhoresVagas(candidatoId, limite = 10) {
     ...
     */
    async buscarMelhoresCandidatos(vagaId, limite = 10) {
