            ...
            {loading && acessibilidades.length === 0 ? (
              ...
              acessibilidades.map((acess) => (
                <TableRow key={acess.id}>
                  <TableCell className="font-medium">{acess.descricao}</TableCell>
                  <TableCell>
                    <span className="text-sm text-muted-foreground">
                      {acess.barreiras?.length || 0} barreira(s)
                    </span>
                  </TableCell>
                        ...
                        <DialogTrigger asChild>
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setSelectedBarreirasIds(acess.barreiras?.map(b => b.id) || []);
                            }}
                          >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <DialogDescription className="text-base">
                                  Selecione as barreiras que <strong>{acess.descricao}</strong> resolve
                                </DialogDescription>
                              </div>
                            </div>
                          </DialogHeader>
                          <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 mt-4">
                            {barreiras.length === 0 ? (
                              <div className="text-center py-8 text-muted-foreground">
                                ...
                                <p>Nenhuma barreira cadastrada</p>
                              </div>
                            ) : (
                              barreiras.map((barreira) => {
                                const isConnected = acess.barreiras?.some(b => b.id === barreira.id);
                                return (
                                  <div
                                    key={barreira.id}
                                    className={`flex items-center justify-between p-4 border-2 rounded-xl transition-all ${
                                      isConnected 
                                        ...
                                        : 'bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-700 hover:border-emerald-200 dark:hover:border-emerald-800 hover:bg-emerald-50/30'
                                    }`}
                                  >
                                    <span className="text-sm font-medium flex-1">{barreira.descricao}</span>
                                    <Button
                                      size="sm"
